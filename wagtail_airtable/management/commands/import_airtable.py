...
class Importer:
    ...
    def get_column_to_field_names(self, airtable_unique_identifier) -> tuple:
        ...
        return airtable_unique_identifier_column_name, airtable_unique_identifier_field_name
    ...
    def convert_mapped_fields(self, record_fields_dict, mapped_fields_dict) -> dict:
        ...
        mapped_fields_dict = {
            mapped_fields_dict[key]: value for (key, value) in record_fields_dict.items() if key in mapped_fields_dict
        }
        return mapped_fields_dict

    def update_object(self, instance, record_id, serialized_data, is_wagtail_model=False) -> bool:
        """
        ...
        if serialized_data.is_valid():
            self.debug_message("\t\t Serializer data was valid. Setting attrs on model...")
            ...
            try:
                if is_wagtail_model:
                    ...
                    if not instance.locked:
                        self.debug_message("\t\t\t Page is not locked. Saving page and creating a new revision.")
                ...
                logger.error(f"Unable to save {instance._meta.label} -> '{obj}'. Error(s): {error_message}")
                self.debug_message(f"\t\t Could not save Wagtail/Django model. Error: {error_message}")
        else:
            logger.info(f"Invalid data for record {record_id}")
            self.debug_message(f"\t\t Serializer was invalid for record: {record_id}, model id: {instance.pk}")
            self.debug_message(f"\t\t Continuing to look for object by its unique identifier: {airtable_unique_identifier_column_name}...")
        # Not updated.
        return False

    def update_object_by_uniq_col_name(self, field_mapping=None, model=None, serialized_data=None, record_id=None, is_wagtail_model=False):
        k, v = zip(*field_mapping.items())
        airtable_unique_identifier_field_name = k[0]
        unique_identifier = v[0]

        if unique_identifier:
            self.debug_message(f"\t\t An Airtable record based on the unique identifier was found: {airtable_unique_identifier_field_name}")
            try:
                obj = model.objects.get(**{airtable_unique_identifier_field_name: unique_identifier})
                self.debug_message(f"\t\t Local object found by Airtable unique column name: {airtable_unique_identifier_field_name}")
            except model.DoesNotExist:
                obj = None
                self.debug_message("\t\t No object was found based on the Airtable column name")
            ...
            if obj:
                ...
                if serialized_data.is_valid():
                    ...
                    try:
                        ...
                        self.debug_message(f"\t\t Unable to save {obj} (ID: {obj.pk}; Airtable Record ID: {record_id}). Reason: {error_message}")
                        self.skipped = self.skipped + 1
                else:
                    logger.info(f"Invalid data for record {record_id}")
                    self.debug_message(f"\t\t Serializer data was invalid.")
                    self.skipped = self.skipped + 1
            else:
                ...
                self.debug_message(f"\t\t {model._meta.verbose_name} with field {airtable_unique_identifier_field_name}={unique_identifier} was not found")
        else:
            ...
            self.debug_message(f"\t\t {model._meta.verbose_name} does not have a unique identifier")
            self.skipped = self.skipped + 1
        return False
    ...
    def is_wagtail_page(self, model):
        if hasattr(model, 'depth'):
            ...
            self.debug_message(f"\t\t {model._meta.verbose_name} is a Wagtail Page and cannot be created from an import.")
            # New record being processed. Save it to the list of records.
            return True
        return False
    ...
    def run(self):
        ...
        for model in models:
            ...
            airtable_unique_identifier_column_name, airtable_unique_identifier_field_name = self.get_column_to_field_names(airtable_settings.get('AIRTABLE_UNIQUE_IDENTIFIER'))
            ...
            self.debug_message(f"\t Airtable base key: {airtable_settings.get('AIRTABLE_BASE_KEY')}")
            self.debug_message(f"\t Airtable table name: {airtable_settings.get('AIRTABLE_TABLE_NAME')}")
            self.debug_message(f"\t Airtable unique identifier settings:")
            self.debug_message(f"\t Airtable records: {len(all_records)}")
            self.debug_message(f"\t\t Airtable column: {airtable_unique_identifier_column_name}")
            self.debug_message(f"\t\t Django field name: {airtable_unique_identifier_field_name}")
            ...
            for record in all_records:
                ...
                # mapped_import_fields = model.map_import_fields(incoming_dict_fields=record_fields) # TODO Check if this param is needed at all
                mapped_import_fields = self.convert_mapped_fields(record_fields, model.map_import_fields())
                ...
                was_updated = self.update_object_by_uniq_col_name(
                    field_mapping={airtable_unique_identifier_field_name: unique_identifier},
                    model=model,
                    serialized_data=serialized_data,
                    record_id=record_id,
                    is_wagtail_model=is_wagtail_model,
                ...
                data_for_new_model = self.get_data_for_new_model(serialized_data, mapped_import_fields, record_id)
                ...
                try:
                    self.debug_message(f"\t\t Attempting to create a new object...")
                    ...
                    new_model.save()
                    self.debug_message(f"\t\t Object created")
                    self.created = self.created + 1
                except ValueError as value_error:
                    logger.info(f"Could not create new model object. Value Error: {value_error}")
                    self.debug_message(f"\t\t Could not create new model object. Value Error: {value_error}")
                except IntegrityError as e:
                    logger.info(f"Could not create new model object. Integrity Error: {e}")
                    self.debug_message(f"\t\t Could not create new model object. Integrity Error: {e}")
                except AttributeError as e:
                    logger.info(f"Could not create new model object. AttributeError. Error: {e}")
                    self.debug_message(f"\t\t Could not create new model object. AttributeError. Error: {e}")
                except Exception as e:
                    logger.error(f"Unhandled error. Could not create a new object for {model._meta.verbose_name}. Error: {e}")
                    self.debug_message(f"\t\t Unhandled error. Could not create a new object for {model._meta.verbose_name}. Error: {e}")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['verbosity'] >= 1:
            self.stdout.write(f"{created} object created. {updated} object updated. {skipped} object skipped.")
