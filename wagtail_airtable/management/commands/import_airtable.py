...
class Importer:
    ...
    def convert_mapped_fields(self, record_fields_dict, mapped_fields_dict):
    ...
    def update_object(self, instance, record_id, serialized_data, is_wagtail_model=False) -> bool:
        ...
        if serialized_data.is_valid():
            ...
            try:
                ...
                print("UPDATED", instance, instance.pk)
                return True
    ...
    def run(self):
        ...
        for model in models:
            ...
            print("====")
            print("RECORDS 1 IS ", len(all_records), " LONG")
            print("records are", all_records)
            print("====")
            ...
            for record in all_records:
                ...
                if unique_identifier:
                    self.debug_message(f"\t\t An Airtable record based on the unique identifier was found: {airtable_unique_identifier_column_name}")

                    try:
                        obj = model.objects.get(**{airtable_unique_identifier_field_name: unique_identifier})
                        self.debug_message(f"\t\t Local object found by Airtable unique column name: {airtable_unique_identifier_column_name}")
                    except model.DoesNotExist:
                        obj = None
                    ...
                    if obj:
                        ...
                        if serialized_data.is_valid():
                            ...
                            try:
                                ...
                                self.records_used.append(record['id'])
                                # New record being processed. Save it to the list of records.
                                continue
                            except ValidationError as error:
                                error_message = '; '.join(error.messages)
        ...
        return self.created, self.skipped, self.updated, self.records_used
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        created, skipped, updated, records_used = importer.run()
